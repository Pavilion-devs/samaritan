...
- [x] Dashboard judge surface implemented with the approved editorial Overview and Live Match, plus evidence-dense Decision Rail, Decisions, Performance, and Proof routes in [`docs/UI.md`](docs/UI.md); captured, historical, synthetic, paper, and unanchored states remain visibly separated.
- [x] Hosted deployment: credentials remain server-side; judges get a [public, read-only, no-login URL](https://getsamaritan.xyz/)
