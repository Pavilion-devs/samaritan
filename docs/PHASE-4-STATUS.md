...
- The implemented paper risk configuration is code-owned: `$50` bankroll, `$3` fixed stake, `$15` aggregate exposure, `$20` drawdown rejection stop, totals-only `CONSENSUS_MOVE`, live-book evidence, and a closed real-money gate. The v1 registration is preserved but suspended; the repaired v2 configuration is not registered.
...
- England-Argentina is scheduled capture-only for the absolute `2026-07-15T16:00:00Z`–`2026-07-15T22:00:00Z` window. Its one-shot supervisor enforces launch skew, startup grace, per-stream freshness, and terminal-manifest checks; a watchdog failure marks the run failed rather than manufacturing coverage. The scheduled run does not itself prove a completed capture, lifecycle coverage, mapping admission, or study eligibility.

## Explicit boundary

This slice starts from a canonical event source or an emitted `DetectorSignal`; code ledgers a signal before model work, runs bounded triage/analysis, and records measured readiness before the scheduler can accept the first later pre-kickoff `PolymarketBookEvent`. Triage and analyst executors remain injected interfaces, with bounded Anthropic implementations and deterministic test doubles. The Claude path has no Polymarket authentication, wallet access, approval, token action, deposit, or order-placement capability. While v2 remains `engineering_candidate_unregistered`, real Anthropic/runtime admission is deliberately blocked; only deterministic-stub or zero-call readiness paths may run.
...
- Execute and verify the England-Argentina capture as capture-only evidence. Do not append it to a study ledger or spend Claude tokens until a corrected v2 protocol is signed and fresh ledgers are initialized.
- France-Spain remains blocked after failed verification; only a fresh capture plus Deborah's corrected v2 registration could reopen admission review.
