# Samaritan Demo Script v1 — Target 4:30–4:45
...
- Record one continuous 1080p browser/terminal walkthrough; target **4:30–4:45** to retain at least 15 seconds of screening margin below the bounty's hard 5:00 limit.
...
- Pre-open: Command, Matchroom, Study, Proof, a terminal at the public repo, and the frozen receipt.
...
| **1:40–2:15** | Open **Study**. Show the invalidated v1 audit, corrected v4 Total Goals panel, registered-forward-paper badge, and the visually separate **Synthetic full-lifecycle proving fixture** panel. Keep `Zero qualifying v2 observations`, `Performance use: Excluded`, and `Solana anchor: Not submitted` visible. | “We also caught our own bad result. The first total-line selector used future information, so I preserved and invalidated v1 instead of hiding it. After repairing causality and economic-case counting, the held-out Total Goals candidate had 38 normalized buy cases across 18 fixtures, plus 132.7 probability basis points after a 100-basis-point proxy, with a fixture-clustered 95 percent interval from plus 14.3 to plus 243.9. These are sampled-price signal results—not alpha, fills, or profitability. I registered the exact corrected configuration for forward paper observation on July 18, before any qualifying v2 observation. The green panel below is a different evidence class: synthetic wiring proof, permanently excluded from those results.” |
...
| **4:00–4:25** | Terminal: run `pnpm public:audit`, then return to **Command → Proof**. | “The judge bundle also passes a public-artifact audit. It contains only a strict allowlist of derived artifacts, with no credentials or wallet controls, no raw TXLine response, and no exact TXLine probability series. Judges can open the dashboard and run the proof without a subscription, API key, or wallet.” |
| **4:25–4:45** | Keep **Proof** and the closed-gate badge visible. Briefly show `Solana anchor: Not submitted`; do not show a placeholder or fake explorer link. | “The devnet prepare, human-gated submit, and read-only verify tools are implemented, but this bounty release intentionally submits no anchor transaction. The receipt is honestly unanchored. Samaritan’s edge is disciplined autonomy: fast deterministic detection, bounded judgment, code-owned risk, and evidence you can challenge. I’m Deborah, the owner and submitter. Thank you.” |
...
- “v2 already has results,” “v2 produced this synthetic fill,” or “registration proves alpha”;
...
- “v2 is registered for forward paper observation only; zero qualifying observations existed at registration”;
...
- [ ] Total runtime is 4:30–4:45 and never exceeds 5:00 after platform processing.
...
- [ ] No placeholder or explorer link appears in the intentionally unanchored release.
