...
Listing checked: [TxODDS — Trading Tools and Agents](https://superteam.fun/earn/listing/trading-tools-and-agents/) on July 14, 2026. The listing requires a public repository, public working MVP, public demo video of at most five minutes, technical documentation with the exact TXLine endpoints, and API feedback.
...
The corrected historical Total Goals candidate produced 38 normalized held-out buy cases across 18 fixtures, `+132.7` probability bps after a 100-bps proxy, with a fixture-clustered 95% interval of `+14.3` to `+243.9` bps. This is sampled-price **signal evidence only**: it is not alpha, executable-fill evidence, or profitability. The v2 paper protocol is unregistered, the bounty build is paper-only, and the real-money gate is closed.
...
- **Study:** v1 invalidated and preserved; corrected v2 is `engineering_candidate_unregistered`.
- **Performance:** no profitability or alpha claim.
- **Solana:** devnet prepare/submit/verify tooling exists, but no transaction has been submitted; the current receipt is unanchored.
...
What worked best was the combination of normalized de-vigged `Pct`, live odds and score SSE, historical interval access, rich score/action envelopes, stable fixture/market identities, and validation surfaces. Together they made a single live/replay event contract possible instead of forcing a demo-only architecture.
...
- [ ] The video is at most five minutes and shows the working product, TXLine's backend role, and the exact proof commands.
- [ ] The final public commit passes install, checks, build, demo, receipt verification, and public-artifact audit.
- [ ] Paper-only, closed-gate, unregistered-v2, unanchored-receipt, and sampled-price limitations remain visible.
