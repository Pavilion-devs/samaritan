...
| Replay/live event contract | “Replay and live ingestion publish the same canonical event types; no replay/live mode field is exposed to strategies.” | Covered by Phase 1 equality tests and Phase 2 archive replay. Claim contract parity, not yet a fully deployed 24/7 autonomous service. |
...
| Actual Claude smoke | “A bounded Haiku triage and Opus thesis path have both completed successfully under the strict schemas.” | Phase 4 records a synthetic research-only smoke. It is not a live trade, performance sample, or end-to-end paper fill. |
| Synthetic lifecycle proof | “A deterministic, closed-world proving fixture traverses canonical event publication, detection, triage and analyst stubs, deterministic risk, pre-action intent, paper depth execution, close, settlement, ledger verification, and receipt generation.” | `pnpm demo` completes at `filled_settled` with 12 v2 ledger rows and zero Anthropic, TXLine API, Polymarket API, wallet, Solana RPC, or real-order calls. It is permanently `excluded_synthetic` and proves wiring—not autonomy quality, alpha, fill rate, or profitability. |
