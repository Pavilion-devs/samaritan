...
> **Submission-owner note:** Samaritan is Deborah's project and submission. Claude is a constrained component of the product, not the participant or submitter. Deborah must personally review this document, rewrite it in her own voice where needed, verify every link against the final public commit, and submit it herself.
...
Further detail: [architecture](../02-architecture.md), [strategy playbook](../03-strategy-playbook.md), [API feedback](txline-api-feedback.md), and the [submission claims matrix](submission-claims-matrix.md).
